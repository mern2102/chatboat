...
const Messagebar = () => {
  ...
  let handleGroupCreate = () => {
    ...
    set(push(refer(db, "group/groupDetails")), {
      groupName: groupName,
  ...
  useEffect(() => {
    const groupListRef = refer(db, "group/groupDetails/");
  ...
  let handleGrpUsersAdd = (uid, name, photo, id) => {
    set(push(refer(db, "group/member")), {
      name: name,
      icon: photo,
      uid: uid,
      groupId: id,
  ...
  useEffect(() => {
    const groupMemListRef = refer(db, "group/member/");
        ...
        <main id="messagesbar_scroll">
          ...
          <div id="friends_list">
            ...
            {groupListBox ? (
              <>
                {groupList.map((data) => (
                  <div onClick={handleGropMsg} key={data.id} style={{ border: "1px solid #e5e5e5", marginBottom: "10px", borderRadius: "8px" }}>
